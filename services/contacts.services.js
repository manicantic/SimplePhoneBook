...
function createContact(userId, contact){
    const query='INSERT INTO contacts (user_id, name, surname, email, birthday, phone) VALUES ($1,$2,$3,$4,$5,$6) RETURNING ID';
    return db.doInsert(query,[userId,contact.name,contact.surname,contact.email,contact.birthday,contact.phone]);
};
...
function updateContact(userId,contactId,contact){
    ...
    return db.doUpdateOrDelete(query,[contact.name,contact.surname,contact.email,contact.birthday,contact.phone,contactId,userId]);
};

function deleteContact(contactId,userId){
    const query='DELETE FROM contacts WHERE id=$1 AND user_id=$2';
    return db.doUpdateOrDelete(query,[contactId,userId]);
};
